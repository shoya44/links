...
| 項目 | 選択 | 理由 |
|------|------|------|
| フレームワーク | **なし（HTML/CSS/JS）** | シンプルさ最優先。ビルド工程を排除し、保守性を高めるため |
...
├── README.md            # 本ドキュメント
└── docs/
    └── CODE_GUIDE.md    # コードを読む人向けの解説（アーキテクチャ・PWA・IndexedDB）
```

> **注意：** `index.html` にすべてのCSS/JSを内包しているのは、iPhoneからでも最短でデプロイできるようにするためです。ファイルを分割する場合は、Service Workerのキャッシュリスト（`STATIC_ASSETS`）も更新してください。

## 機能

| 機能 | 概要 |
|------|------|
| リンクを開く | 一覧をタップで別タブに開く。Google系URLは[特別処理](#1-google関連urlの特別処理) |
| 編集モード | 追加・削除・並び替え・アイコン変更。**Save** で確定、**Cancel** で破棄（未保存の変更があれば確認） |
| 一括追加 | URL欄に複数行のURLをペーストすると、1行ずつ行が作られる |
| タイトル補完 | タイトルが空なら保存時にホスト名（`www.` を除く）で補完。編集中はプレースホルダで予告表示 |
| 入力チェック | 不正なURL・重複URLの行は赤枠で示し、保存を中断する（空欄の行は黙って捨てる） |
| バックアップ | 編集モード下部の **Export** でJSONを共有シートに書き出し、**Import** でJSONまたは1行1URLのテキストから取り込み（重複URLはスキップ） |
...
```js
{
  ...
  updatedAt: number    // Unix timestamp（内容が変わった時だけ更新）
}
```
...
**保存方法：** Saveのたびにストアを `clear()` してから全件 `put()` する（1トランザクション）。`url` に一意インデックスがあるため、個別に put/delete すると順序次第で制約エラーになるのを避けるため。

**アイコン仕様：**

- `icon` が `SVG_ICONS` のキー名（例：`'file'`, `'table'`）の場合、内蔵SVGを表示
- `icon` が `http://`、`https://`、`data:` で始まる場合は画像として表示（旧データ互換用。読み込み失敗時は `'link'` にフォールバック）
- `icon` が空または未設定の場合は `'link'` SVGアイコンを表示

## バックアップファイルの形式

Exportで書き出されるJSONは次の形式です。Importはこの形式のほか、`links` 配列だけのJSON、1行1URLのプレーンテキストも受け付けます。
...
```json
{
  ...
  "links": [
    { "url": "https://example.com", "title": "Example", "icon": "globe" }
  ]
}
```
...
| その他の `google.com` / `google.co.jp` のURL | `accounts.google.com/AccountChooser` を経由してアカウント選択画面を表示 |

対応表は `GOOGLE_APP_SCHEMES` にまとめてあり、追加する場合はそこに1行足すだけです。

**実装上の注意：**

- アプリスキームで開いた後、1.5秒後（`APP_SCHEME_FALLBACK_MS`）に `document.visibilityState === 'visible'` の場合のみブラウザでフォールバック表示する
...
- アイコンを追加する場合は、`SVG_ICONS` に追記すれば `ICON_NAMES` とピッカーが自動的に更新される
- 現在選択中のアイコンは青背景でハイライトされる
- 背景タップ・キャンセル・Escapeキーで閉じる
...
実装の要点（`attachDragHandle` / `beginDrag`、`index.html` 内）：

- `DRAG_DELAY_MS`（200ms）: タッチ操作のみ、長押し判定を挟んでからドラッグ開始する。マウス/ペンは即座に開始
...
- ドラッグ中は全行のフェードインアニメーションを止める（再生中だと、そのアニメーションが `transform` を上書きして追従できないため）
...
### 5. 入力欄（iPhoneのキーボード対応）

- URL欄は `inputmode="url"`、`autocapitalize="off"`、`autocorrect="off"`、`spellcheck="false"` を指定し、先頭の自動大文字化や自動修正でURLが壊れないようにしている
- タイトル欄は「次へ」（`enterkeyhint="next"`）でURL欄へ移動、URL欄は「完了」（`enterkeyhint="done"`）でキーボードを閉じる
- URL欄からフォーカスが外れた時と保存時に `https://` を補完する（`normalizeUrlInput`）
...
- Service Workerのキャッシュバージョン（`CACHE_NAME`）は、ファイルを更新するたびに変更すること（例：`v6` → `v7`）。ページ本体はネットワーク優先で取得するため更新は届くが、オフライン用キャッシュと `manifest.json` はこの名前で世代管理している

## 既知の制約と注意点

### iOSのPWA制約

- **7日ルール：** 7日間PWAを起動しないと、IndexedDBとCacheが削除される可能性がある（ホーム画面に追加したPWAは比較的安全だが、定期的な起動を推奨）。念のため **Export** で定期的にバックアップを取ること
...
- **共有シート：** Exportは `navigator.share` でファイル共有できる環境（iOS 15以降など）では共有シートを開き、それ以外ではファイルをダウンロードする

### 外部依存

- なし（並び替え・アイコンを含め、外部ライブラリ・CDNには依存していない）
- Google Favicon API は使用していない（自動ファビコン取得を廃止）。`icon` に画像URLを入れた旧データは引き続き画像として表示される
...
- コードの読み方は [docs/CODE_GUIDE.md](docs/CODE_GUIDE.md) を参照
